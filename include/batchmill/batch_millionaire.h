...
template <typename IO> class BatchMillionaireProtocol {
public:
    IO* io = nullptr;
    sci::KKOT<IO>* kkot;
    BatchKKOT<IO>* batch_kkot;
    ...
    BatchMillionaireProtocol(int party, IO* io, sci::KKOT<IO>* kkot, sci::OTPack<IO>* otpack,
        int bitlength = 32, int radix_base = MILL_PARAM) {
        this->party = party;
        this->io = io;
        this->kkot = kkot;
        this->triple_gen = new TripleGenerator<IO>(party, io, otpack);
        this->batch_kkot = new BatchKKOT<IO>(io, kkot);
    ...
    ~BatchMillionaireProtocol() { 
        delete triple_gen; 
        delete batch_kkot;
    }

    void batch_compare(uint8_t* res, uint64_t* data, uint64_t cmp, int num_cmps, int bitlength,
        bool greater_than = true, int radix_base = MILL_PARAM) {
        configure(bitlength, radix_base);
        ...
        for (int i = 0; i < num_digits; i++) { // Stored from LSB to MSB 
            if (party == sci::ALICE) {
                ...
                if ((i == num_digits - 1) && (r != 0))
                    choice[i] =
                    (uint8_t)(cmp >> i * beta) & mask_r;
                else
                    choice[i] =
                    (uint8_t)(cmp >> i * beta) & mask_beta;
            }
        }

        uint64_t* N = new uint64_t[num_digits];
        ...
        if (party == sci::ALICE) {
            ...
            // Perform Leaf OTs
            //batch_kkot->send_batch_kkot(leaf_ot_messages, N, num_digits, num_cmps, 2);
            batch_kkot->kkot->N = N[0];
            batch_kkot->kkot->send_pre(num_digits);
            batch_kkot->send_batch_kkot_post(leaf_ot_messages, N, 1, num_cmps, 1);
            for (int i = 0; i < num_digits - 1; i++) {
                batch_kkot->kkot->qT[i] = batch_kkot->kkot->qT[i + 1];
            }
            batch_kkot->send_batch_kkot_post(leaf_ot_messages + num_cmps, N + 1, num_digits - 1, num_cmps, 2);
        ...
        {
            // Perform Leaf OTs
            //batch_kkot->recv_batch_kkot(leaf_res_cmp, N, choice, num_digits, num_cmps, 2);
            batch_kkot->kkot->N = N[0];
            batch_kkot->kkot->recv_pre(choice, num_digits);
            batch_kkot->recv_batch_kkot_post(leaf_res_cmp, N, choice, 1, num_cmps, 1);
            for (int i = 0; i < num_digits - 1; i++) {
                batch_kkot->kkot->tT[i] = batch_kkot->kkot->tT[i + 1];
            }
            batch_kkot->recv_batch_kkot_post(leaf_res_cmp + num_cmps, N + 1, choice + 1, num_digits - 1, num_cmps, 2);

            /*for (int i = 0; i < num_digits; i++) {
                cout << (uint64_t)choice[i] << " ";
            }
            cout << endl;*/

            // Extract equality result from leaf_res_cmp
            for (int i = num_cmps; i < num_digits * num_cmps; i++) {
                /*if ((i % num_cmps) < old_num_cmps) {
                    cout << (uint64_t)(leaf_res_cmp[i] & 3) << "=>";
                }*/
                leaf_res_eq[i] = leaf_res_cmp[i] & 1;
                leaf_res_cmp[i] >>= 1;
                /*if ((i % num_cmps) < old_num_cmps) {
                    cout << (uint64_t)leaf_res_cmp[i] << " " << (uint64_t)leaf_res_eq[i] << endl;
                }*/
            }
        }
        ...
        memcpy(res, leaf_res_cmp, old_num_cmps * sizeof(uint8_t));
...
#if USE_CHEETAH
    std::cout << "use silent triple" << std::endl;
...
#else
        std::cout << "use iknp triple" << std::endl;
        triple_gen->generate(party, &triples_corr, _8KKOT);
        triple_gen->generate(party, &triples_std, _16KKOT_to_4OT);
#endif
        // std::cout << "Bit Triples Generated" << std::endl;
        pre_comm = io->counter - comm;
        pre_time = std::chrono::duration_cast<std::chrono::microseconds>(high_resolution_clock::now() - time).count();
